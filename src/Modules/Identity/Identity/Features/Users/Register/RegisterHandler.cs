...
public class RegisterHandler(UserManager<IdentityUser> userManager, IdentityDbContext dbContext) : ICommandHandler<RegisterCommand, Guid>
{
    public async Task<Guid> HandleAsync(RegisterCommand command, CancellationToken ct = default)
    {
        ...
        if (result.Succeeded)
        {
            var outbox = new OutboxMessage
            {
                ...
                Content = JsonSerializer.Serialize(new UserRegisteredEvent(Guid.Parse(user.Id), user.Email, user.UserName)),
