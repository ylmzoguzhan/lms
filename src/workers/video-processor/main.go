...
func handleVideoProcessing(event VideoUploadedEvent) error {
	...
	if err != nil {
		return fmt.Errorf("dosya istatistiği alınamadı: %w", err)
	}
	log.Printf("📥 İndirilen dosya boyutu: %d bytes", fileInfo.Size())

	if fileInfo.Size() == 0 {
		return fmt.Errorf("indirilen dosya boş (0 bytes)! Muhtemelen MinIO'da dosya henüz yok.")
	}
